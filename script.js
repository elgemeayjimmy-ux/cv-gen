...
// تهيئة السيرفر (تأكد إنك ضايف مكتبات فايربيز في الـ HTML)
...
async function aiGenerateArticleReal() {
    ...
    canvas.innerHTML = `<h3 style="color:#888;">AI is writing your document... please wait ⏳</h3>`;
...
// --- [6] العروض التقديمية (PowerPoint Generator) ---
let pptSlides = [];
...
function addSlidePreview() {
    ...
    if(list) {
        ...
        <div style="background:var(--bg-main); padding:15px; border-radius:8px; border:1px solid var(--border-color);">
            ...
            <p style="font-size:12px; color:var(--text-muted); margin-top:5px;">${content}</p>
        </div>`;
    }
...
function exportPPTX() {
    ...
    slideTitle.addText("My Awesome Presentation", { x:1.5, y:1.5, fontSize:36, color:"363636", bold:true });
    slideTitle.addText("Generated by Ultimate OS Workstation", { x:1.5, y:2.5, fontSize:18, color:"666666" });
...
// --- [10] محرك Team Mode اللحظي (Live Firebase) ---
let currentUser = "";
...
// الاستماع للتغييرات الحية (Live Sync)
function listenToRoomSync() {
    if(!currentRoom) return;

    // مزامنة الأعضاء المتصلين
    ...
    // مزامنة الكتابة في الـ CV
    database.ref('rooms/' + currentRoom + '/cvData').on('value', (snapshot) => {
        const data = snapshot.val();
        // لو التعديل جاي من حد تاني غيري، أحدث الشاشة عندي
        if(data && data.updatedBy !== currentUser) {
            ...
            generateCV(); // تحديث الورقة اللحظي
        }
    });
}

// دالة لرفع التعديلات للسيرفر اللحظي
